...
func main() {
	remoteAPIURL := []string{
		"https://myinstantpower.com/safety-data-sheets/",
		"https://myinstantpower.com/ingredients-disclosure/",
	}
	localFilePath := "30secondscleaners.html" // Path where HTML file will be stored
	var getData string                        // Variable to hold HTML content

	for _, urls := range remoteAPIURL {
		getData = scrapePageHTMLWithChrome(urls)     // If not, download HTML content from URL
...
func extractPDFUrls(html string) []string {
	...
	// Find all <a> elements and check href attribute
	doc.Find("a").Each(func(i int, s *goquery.Selection) {
		href, exists := s.Attr("href")
		if exists && strings.HasSuffix(strings.ToLower(href), ".pdf") {
			pdfUrls = append(pdfUrls, href)
		}
	})
